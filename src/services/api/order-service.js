...
class OrderService {
    constructor() {
        this.axiosService = new AxiosService(`${config.USER_BASE_URL}/api/v1/order`, 'order');
        axiosOrderInstance = this.axiosService.axios;
    }

    async getOrderById(orderId) {
        const response = await axiosOrderInstance.get(`/id/${orderId}`);
        return response;
    }
    ...
    async requestDeliveryDateExtension( orderId, body) {
        const response = await axiosOrderInstance.put(`/extension/${orderId}`, { paymentIntentId, body});
        return response;
    }
    ...
    //flag
    async deliverOrder( orderId,type, body) {
        const response = await axiosOrderInstance.put(`/deliver-order/${orderId}/${type}`, body);
        return response;
    }
